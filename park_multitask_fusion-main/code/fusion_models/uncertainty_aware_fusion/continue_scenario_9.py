#!/usr/bin/env python
"""
Continue running scenario 9 from a specific seed
...
def run_experiment(seed):
    ...
    # Run the uncertainty_aware_fusion_scenario_9.py with the given seed
    cmd = [
        sys.executable,
        "uncertainty_aware_fusion_scenario_9.py",
        f"--seed={seed}"
    ]
...
def collect_results():
    ...
    if os.path.exists('fusion_model_results_test.json'):
        try:
            ...
            print(f"Warning: Could not read test results JSON: {e}")
    ...
    if os.path.exists('fusion_model_results_dev.json'):
        try:
            ...
            print(f"Warning: Could not read dev results JSON: {e}")
...
def find_latest_results_dir():
    """Find the latest scenario_9_results directory"""
    dirs = glob.glob("scenario_9_results_*")
    if not dirs:
        return None
    return sorted(dirs)[-1]

def main():
    # Find existing results directory
    results_dir = find_latest_results_dir()
    
    if results_dir:
        print(f"\n{'='*70}")
        print(f"Found existing results directory: {results_dir}")
        
        # Find which seeds have already been completed
        completed_seeds = set()
        test_files = glob.glob(f"{results_dir}/test_seed_*.json")
        for f in test_files:
            seed_num = int(f.split('_')[-1].replace('.json', ''))
            completed_seeds.add(seed_num)
        
        print(f"Already completed seeds: {sorted(completed_seeds)}")
        print(f"Total completed: {len(completed_seeds)}")
        
        start_seed = max(completed_seeds) + 1 if completed_seeds else 0
        print(f"Continuing from seed: {start_seed}")
        print(f"{'='*70}\n")
    else:
        print(f"\n{'='*70}")
        print(f"No existing results found, starting from scratch")
        print(f"{'='*70}\n")
        results_dir = f"scenario_9_results_continue"
    ...
    all_test_metrics = []
    all_dev_metrics = []
    successful_seeds = list(completed_seeds)
    failed_seeds = []
    ...
    for seed in completed_seeds:
        ...
        if os.path.exists(test_file):
            with open(test_file, 'r') as f:
                all_test_metrics.append(json.load(f))
        
        if os.path.exists(dev_file):
            with open(dev_file, 'r') as f:
                all_dev_metrics.append(json.load(f))
    
    # Continue running experiments
    for seed in range(start_seed, 101):  # Continue to 100

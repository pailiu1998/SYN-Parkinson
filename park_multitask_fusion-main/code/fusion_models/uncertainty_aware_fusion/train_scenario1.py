...
from constants_chinese_scenario1 import *
...
def load_smile_data(drop_correlated = True, corr_thr = 0.85):
    ...
    # Chinese data already has ID and date columns
    df["id"] = df['ID']
    if 'date' not in df.columns:
        df["date"] = df.Filename.apply(parse_date)
    df["id_date"] = df["id"].astype(str)+"#"+df["date"].astype(str)
...
def load_qbf_data(drop_correlated = False, corr_thr = 0.85, feature_files=[AUDIO_FEATURES_FILE]):
    ...
    # Parse ID and date (Chinese data has ID column)
    if 'ID' in df.columns:
        df["id"] = df["ID"]
    else:
...
def load_finger_data(hand="left",drop_correlated = False, corr_thr = 0.85):
    ...
    #Drop metadata columns to focus on features (errors='ignore' for Chinese data compatibility)
    df_features = df.drop(columns=['Unnamed: 0','filename','Protocol','Participant_ID','Task',
                'Duration','FPS','Frame_Height','Frame_Width','gender','age','race',
                'ethnicity','pd','dob','time_mdsupdrs','hand','date'], errors='ignore')
    '''
    Drop columns (if set true) if it is correlated with another one with PCC>thr
    '''
    if drop_correlated:
    ...
    # Parse ID and date (Chinese data already has these columns)
    ...
    df["id_date"] = df["id"].astype(str)+"#"+df["date"].astype(str)+"#"+df["hand"].astype(str)
    df["label"] = 1.0*(df["pd"]!="no")
...
def concat_finger_features(row):
    # Return as list to avoid pandas DataFrame issue
    return list(np.concatenate([row[f"features_right"], row[f"features_left"]]))
...
def evaluate_fusion_model(fusion_model, dataloader, prediction_models, config, split="dev"):
    ...
    # Save predictions and labels to JSON
    output_data = {
        "prediction": all_final_predictions.tolist(),
        "label": all_labels.tolist()
    }
    output_filename = f"fusion_model_results_{split}.json"
...
# quick+smile randomseed
# @click.command()
# @click.option("--learning_rate", default=0.3755250534938169, help="Learning rate for classifier")
# @click.option("--dropout_prob", default=0.1514516567036872)
# @click.option("--num_buckets", default=5, help="Options: 5, 10, 20, 50, 100")
# @click.option("--num_trials", default=30, help="Options: 100-1000")
# @click.option("--uncertainty_weight", default=60.48662805348768)
# @click.option("--minority_oversample", default='no', help="Options: 'yes', 'no'")
...
# @click.option("--num_epochs", default=69)
# @click.option("--hidden_dim", default=128)
...
# @click.option("--beta2", default=0.999)
# @click.option("--weight_decay", default=0.0001)
# @click.option("--momentum", default=0.38289922861439807)
# @click.option("--use_scheduler", default='yes', help="Options: yes, no")
# @click.option("--scheduler", default='reduce', help="Options: step, reduce")
# @click.option("--step_size", default=18)
# @click.option("--gamma", default=0.5713133675520001)
# @click.option("--patience", default=14)


# second best new syn fusion
# @click.command()
# @click.option("--learning_rate", default=0.3591653287336956, help="Learning rate for classifier")
# @click.option("--dropout_prob", default=0.4820540501688951, help="Dropout probability")
# @click.option("--num_buckets", default=50, help="Options: 5, 10, 20, 50, 100")
# @click.option("--num_trials", default=30, help="Options: 100-1000")
# @click.option("--uncertainty_weight", default=57.41429660820699, help="Uncertainty weight")
# @click.option("--minority_oversample", default='no', help="Options: 'yes', 'no'")
# @click.option("--sampler", default='SMOTEN', help="Options: SMOTE, SMOTENC, SVMSMOTE, ADASYN, BorderlineSMOTE, KMeansSMOTE, SMOTEN, RandomOverSampler, SMOTEENN, SMOTETomek")
# @click.option("--train_random_noise", default="no", help="Options: yes, no")
# @click.option("--validation_random_noise", default="no", help="Options: yes, no")
# @click.option("--increase_variance", default="no", help="Options: yes, no")
# @click.option("--temperature", default=0.05, help="Float between 0 and 1")
...
# @click.option("--num_epochs", default=179, help="Number of epochs")
# @click.option("--hidden_dim", default=32, help="Dimension of hidden layers")
# @click.option("--query_dim", default=128, help="Dimension of query vector")
# @click.option("--last_hidden_dim", default=8, help="Dimension of last hidden layer")
# @click.option("--optimizer", default="SGD", help="Options: SGD, AdamW, RMSprop")
# @click.option("--beta1", default=0.9, help="Beta1 parameter for optimizer")
# @click.option("--beta2", default=0.999, help="Beta2 parameter for optimizer")
# @click.option("--weight_decay", default=0.0001, help="Weight decay for regularization")
# @click.option("--momentum", default=0.6857200786090252, help="Momentum for optimizer")
# @click.option("--use_scheduler", default='yes', help="Options: yes, no")
# @click.option("--scheduler", default='reduce', help="Options: step, reduce")
# @click.option("--step_size", default=13, help="Step size for scheduler")
# @click.option("--gamma", default=0.7819821715017687, help="Gamma parameter for scheduler")
# @click.option("--patience", default=17, help="Patience for early stopping")

# best train synthetic park finger
# @click.command()
# @click.option("--learning_rate", default=0.21246102146346243, help="Learning rate for classifier")
# @click.option("--dropout_prob", default=0.3845439290217162, help="Dropout probability")
# @click.option("--num_buckets", default=20, help="Options: 5, 10, 20, 50, 100")
# @click.option("--num_trials", default=30, help="Options: 100-1000")
# @click.option("--uncertainty_weight", default=85.09629545785756, help="Uncertainty weight")
# @click.option("--minority_oversample", default='no', help="Options: 'yes', 'no'")
# @click.option("--sampler", default='SMOTEENN', help="Options: SMOTE, SMOTENC, SVMSMOTE, ADASYN, BorderlineSMOTE, KMeansSMOTE, SMOTEN, RandomOverSampler, SMOTEENN, SMOTETomek")
# @click.option("--train_random_noise", default="no", help="Options: yes, no")
# @click.option("--validation_random_noise", default="no", help="Options: yes, no")
# @click.option("--increase_variance", default="no", help="Options: yes, no")
# @click.option("--temperature", default=0.05, help="Float between 0 and 1")
...
# @click.option("--num_epochs", default=286, help="Number of epochs")
# @click.option("--hidden_dim", default=8, help="Dimension of hidden layers")
# @click.option("--query_dim", default=128, help="Dimension of query vector")
# @click.option("--last_hidden_dim", default=8, help="Dimension of last hidden layer")
# @click.option("--optimizer", default="SGD", help="Options: SGD, AdamW, RMSprop")
# @click.option("--beta1", default=0.9, help="Beta1 parameter for optimizer")
# @click.option("--beta2", default=0.999, help="Beta2 parameter for optimizer")
# @click.option("--weight_decay", default=0.0001, help="Weight decay for regularization")
# @click.option("--momentum", default=0.12777708825498754, help="Momentum for optimizer")
# @click.option("--use_scheduler", default='no', help="Options: yes, no")
# @click.option("--scheduler", default='reduce', help="Options: step, reduce")
# @click.option("--step_size", default=3, help="Step size for scheduler")
# @click.option("--gamma", default=0.848325749314616, help="Gamma parameter for scheduler")
# @click.option("--patience", default=11, help="Patience for early stopping")


# #  best new syn fusion
# @click.command()
# @click.option("--learning_rate", default=0.4939279896381278, help="Learning rate for classifier")
# @click.option("--dropout_prob", default=0.47558192399389226, help="Dropout probability")
# @click.option("--num_buckets", default=10, help="Options: 5, 10, 20, 50, 100")
# @click.option("--num_trials", default=30, help="Options: 100-1000")
# @click.option("--uncertainty_weight", default=43.14948430384826, help="Uncertainty weight")
# @click.option("--minority_oversample", default='no', help="Options: 'yes', 'no'")
# @click.option("--sampler", default='SMOTETomek', help="Options: SMOTE, SMOTENC, SVMSMOTE, ADASYN, BorderlineSMOTE, KMeansSMOTE, SMOTEN, RandomOverSampler, SMOTEENN, SMOTETomek")
# @click.option("--train_random_noise", default="no", help="Options: yes, no")
# @click.option("--validation_random_noise", default="no", help="Options: yes, no")
...
# @click.option("--seed", default=113, help="Seed for random")
# @click.option("--batch_size", default=256, help="Batch size")
# @click.option("--num_epochs", default=239, help="Number of epochs")
# @click.option("--hidden_dim", default=16, help="Dimension of hidden layers")
# @click.option("--query_dim", default=32, help="Dimension of query vector")
# @click.option("--last_hidden_dim", default=8, help="Dimension of last hidden layer")
# @click.option("--optimizer", default="SGD", help="Options: SGD, AdamW, RMSprop")
# @click.option("--beta1", default=0.9, help="Beta1 parameter for optimizer")
# @click.option("--beta2", default=0.999, help="Beta2 parameter for optimizer")
# @click.option("--weight_decay", default=0.0001, help="Weight decay for regularization")
# @click.option("--momentum", default=0.9352449728654904, help="Momentum for optimizer")
# @click.option("--use_scheduler", default='yes', help="Options: yes, no")
# @click.option("--scheduler", default='step', help="Options: step, reduce")
# @click.option("--step_size", default=10, help="Step size for scheduler")
# @click.option("--gamma", default=0.8165622914216544, help="Gamma parameter for scheduler")
# @click.option("--patience", default=4, help="Patience for early stopping")
...
# @click.option("--beta1",default=0.9)
# @click.option("--beta2",default=0.999)
# @click.option("--dropout_prob", default=0.19987176295964623)
# @click.option("--gamma",default=0.7185599601091909)
# @click.option("--hidden_dim", default=4)
# @click.option("--increase_variance",default="no", help="Options: yes, no")
# @click.option("--last_hidden_dim", default=8)
# @click.option("--learning_rate", default=0.13653689137329014, help="Learning rate for classifier")
# @click.option("--minority_oversample",default='no',help="Options: 'yes', 'no'")
# @click.option("--model_subset_choice", default=0, help="4 possible choices. See Constants.py")
# @click.option("--momentum",default=0.7753063595176392)
# @click.option("--noise_variance",default=0.01,help="Float between 0 and 1")
# @click.option("--num_buckets", default=10, help="Options: 5, 10, 20, 50, 100")
# @click.option("--num_epochs",default=150)
# @click.option("--num_trials", default=30, help="Options: 100-1000")
# @click.option("--optimizer",default="SGD",help="Options: SGD, AdamW, RMSprop")
# @click.option("--patience",default=20)
# @click.option("--query_dim", default=32)
# @click.option("--random_state", default=171, help="Random state for classifier")
# @click.option("--sampler", default='SMOTEENN', help="Options:SMOTE, SMOTENC, SVMSMOTE, ADASYN, BorderlineSMOTE, KMeansSMOTE, SMOTEN, RandomOverSampler, SMOTEENN, SMOTETomek")
# @click.option("--scheduler",default='step',help="Options: step, reduce")
# @click.option("--seed", default=113, help="Seed for random")
# @click.option("--step_size",default=17)
# @click.option("--temperature", default=0.05, help="Float between 0 and 1")
# @click.option("--train_random_noise", default="no", help="Options: yes, no")
# @click.option("--uncertainty_weight", default=67.48074248533312)
# @click.option("--use_scheduler",default='no',help="Options: yes, no")
# @click.option("--validation_random_noise", default="no", help="Options: yes, no")
# @click.option("--weight_decay",default=0.0001)
...
# masked VAE
# @click.command()
# @click.option("--learning_rate", default=0.2776314202708178, help="Learning rate for classifier")
# @click.option("--dropout_prob", default=0.40935720262731584, help="Dropout probability")
# @click.option("--num_buckets", default=5, help="Options: 5, 10, 20, 50, 100")
# @click.option("--num_trials", default=30, help="Options: 100-1000")
# @click.option("--uncertainty_weight", default=24.8967195910124, help="Uncertainty weight")
# @click.option("--minority_oversample", default='no', help="Options: 'yes', 'no'")
# @click.option("--sampler", default='SMOTEENN', help="Options: SMOTE, SMOTENC, SVMSMOTE, ADASYN, BorderlineSMOTE, KMeansSMOTE, SMOTEN, RandomOverSampler, SMOTEENN, SMOTETomek")
# @click.option("--train_random_noise", default="no", help="Options: yes, no")
# @click.option("--validation_random_noise", default="no", help="Options: yes, no")
...
# @click.option("--num_epochs", default=231, help="Number of epochs")
# @click.option("--hidden_dim", default=8, help="Dimension of hidden layers")
# @click.option("--query_dim", default=128, help="Dimension of query vector")
# @click.option("--last_hidden_dim", default=8, help="Dimension of last hidden layer")
# @click.option("--optimizer", default="SGD", help="Options: SGD, AdamW, RMSprop")
...
# @click.option("--beta2", default=0.999, help="Beta2 parameter for optimizer")
# @click.option("--weight_decay", default=0.0001, help="Weight decay for regularization")
# @click.option("--momentum", default=0.8682367756982771, help="Momentum for optimizer")
# @click.option("--use_scheduler", default='no', help="Options: yes, no")
# @click.option("--scheduler", default='reduce', help="Options: step, reduce")
# @click.option("--step_size", default=7, help="Step size for scheduler")
# @click.option("--gamma", default=0.8699716688419532, help="Gamma parameter for scheduler")
# @click.option("--patience", default=2, help="Patience for early stopping")
...
def main(**cfg):
    ...
    ENABLE_WANDB = True
    if ENABLE_WANDB:
        wandb.init(project="park_chinese_scenario1", config=cfg)
    ...
    # selected_models = ['fox_model_best_auroc_baal', 'facial_expression_smile_best_auroc_baal']
    ...
    for i in range(NUM_MODELS):
        ...
        # path["SCALER"] = os.path.join(MODEL_BASE_PATH, MODEL_TAG,"scaler") # There are some checkpoints don't have scaler
    ...
    for i in range(NUM_MODELS):
        ...
        if "finger_model" in model_name:
            ...
            # Manually concat features to avoid pandas apply issues
            features_list = []
    ...
    print("test_df")
    print(test_df["id"].tolist())
    print("dev_df")
    print(dev_df["id"].tolist())
    ...
    save_dir = os.path.join(BASE_DIR, "models/fusion_scenario1")
    os.makedirs(save_dir, exist_ok=True)
    torch.save(best_model.state_dict(), os.path.join(save_dir, "fusion_model.pth"))
